<template>
  <div class="loadmore">
    <div
      ...
      :style="{
        transform: 'translateY(' + (canRef ? translate : '0') + 'px)',
        'transition-duration': animateTime + 's'
      }">
      <slot name="top">
        <div v-if="topMethod" class="top">
          <v-spinner v-if="topStatus != ''" type="snake" width="20" height="20" color="#aeaeae"></v-spinner>
          <p style="display: inline; text-align: center">{{ topText }}</p>
        </div>
      </slot>
      <slot></slot>
      <slot v-if="bottomType === 'drop' && canInif" name="bottom">
        <div v-if="bottomMethod" class="bottom">
          <v-spinner width="20" type="snake" color="#aeaeae"></v-spinner>
          <span class="text">{{ bottomText }}</span>
        </div>
      </slot>
      <slot v-if="canInif && bottomType != 'drop'" name="bottom">
          ...
          this.$nextTick(() => {
            if (this.scrollEventTarget === window) {
              this.containerFilled =
                this.$el.getBoundingClientRect().bottom >= document.documentElement.getBoundingClientRect().bottom;
            } else {
              this.containerFilled =
      ...
      scrollListener: function (event) {
        ...
        if (scrollEventTarget === element) {
          ...
          const elementBottom =
            ...
            this.getElementTop(scrollEventTarget) +
