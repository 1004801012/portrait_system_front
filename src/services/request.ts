...
const axiosResponseConfig = {
  success (response) {
    if (response && TOKEN_EXPIRE.includes(response.data.code)) {
      removeToken()
...
export default function request (url, {
  ...
  let formatHeaders = Object.assign({
  ...
  if (method === 'get') {
    ...
    if (typeof contentType !== 'undefined') {
      if (contentType.indexOf('multipart') !== -1) {
        ...
        defaultConfig.data = Qs.stringify(data, { indices: false })
      }
    }
  }
